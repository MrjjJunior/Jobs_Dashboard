...
[Your Name]`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-pine-900/50 backdrop-blur-xs animate-fadeIn">
      <div className="bg-white rounded-2xl max-w-xl w-full border border-linen-200 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="px-6 py-4 bg-pine-600 text-white flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-pine-100">Draft professional follow-ups and thank-you emails in seconds</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Controls */}
        <div className="p-5 border-b border-linen-100 bg-linen-50 space-y-3 text-xs">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-1.5 bg-white border border-linen-200 rounded-lg text-xs font-semibold text-pine-800 focus:outline-none focus:border-pine-500"
              >
                {jobs.map((j) => (
        ...
        <div className="p-5 flex-1 overflow-y-auto">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[11px] font-bold text-taupe-500 uppercase tracking-wider">
              Generated Message Preview
            </span>
            <span className="text-[10px] text-taupe-400 font-medium">Ready to customize & send</span>
          </div>

          <div className="p-4 bg-linen-50 border border-linen-200 rounded-xl font-mono text-xs text-pine-800 whitespace-pre-wrap leading-relaxed">
            {draftText}
          </div>
        </div>

        {/* Footer */}
        <div className="px-6 py-3.5 bg-white border-t border-linen-200 flex items-center justify-between text-xs">
          <button
            onClick={onClose}
          ...
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 px-4 py-2 bg-pine-600 hover:bg-pine-700 text-white rounded-lg font-bold shadow-xs transition-colors"
          >
            {copied ? (
              <>
                <Check className="w-3.5 h-3.5 text-olive-300" />
